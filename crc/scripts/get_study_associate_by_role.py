...
from crc.models.study import StudyAssociated, StudyAssociatedSchema
from crc.scripts.script import Script # pylint disable=unable-to-import


class GetAssociateByRole(Script):
    ...
    def do_task(self, task, study_id, workflow_id, *args, **kwargs):  # pylint: disable=unused-argument
        ...
        if 'role' in kwargs:
            associate = (session.query(StudyAssociated).filter(StudyAssociated.study_id == study_id).
                  filter(StudyAssociated.role == kwargs['role']).
                  first())
            if associate:
                return StudyAssociatedSchema().dump(associate)
            else:
                return StudyAssociatedSchema().dump(None)
        raise ApiError(code='missing_role', message='get_associate_by_role requires a role')
